...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NgxEchartsModule, ChartSwitcherComponent],
  templateUrl: './analysis-block.component.html'
})
export class AnalysisBlockComponent implements OnInit, OnDestroy {
  @Input() taskId!: number;
  ...
  startPolling() {
    if (!this.pollSubscription) {
      this.pollSubscription = interval(2000).subscribe(() => {
        this.api.getAnalysisTask(this.taskId).subscribe({
          next: (task) => {
            this.task = task;
    ...
    this.api.getAnalysisTask(this.taskId).subscribe({
      next: (task) => {
        ...
        if (task.status === 'COMPLETED') {
          ...
          // Task is pending or started, start polling
          this.startPolling();
        }
      },
      error: () => {
  ...
  renderBoxplot() {
    ...
    for (const stat of this.statsTable) {
      if (stat.min !== undefined && stat['25%'] !== undefined && stat['50%'] !== undefined && stat['75%'] !== undefined && stat.max !== undefined) {
        ...
        boxplotData.push([
          stat.min, stat['25%'], stat['50%'], stat['75%'], stat.max
        ]);
      }
    }
